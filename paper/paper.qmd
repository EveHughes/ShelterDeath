...
In the following section, I will present a series of visualizations that analyze the number of deaths among shelter residents over time, highlighting the gender distribution and overall data trends. These charts will provide a clearer understanding of the data composition and allow for an insightful examination of how the number of deaths in this population has evolved. There are 212 observations in the original dataset in total and there is no special cleaning methods needed as the data is already clear enough. See Appendix @Appendix for the summary of the dataset.
...
ggplot(shelter_death_data, aes(x = month, y = total_decedents, group = year)) +
  ...
  labs(title = "Deaths in Toronto Shelters Over Time (2007.01-2024.08)",
       x = "Month",
...
ggplot(annual_death_counts_bar, aes(x = factor(year), y = count, fill = gender)) +
  ...
  labs(title = "Annual Death Counts in Toronto Shelters With Gender Distribution (2007-2023)",
       x = "year",
       y = "num of death",
       fill = "gender") +
...
ggplot(annual_deaths, aes(x = year, y = total_yearly_deaths)) +
  ...
  labs(title = "Annual Deaths in Toronto Shelters (2007-2023)",
       x = "Year",
       y = "Total Deaths") +  # add title and name of the axis
  theme_minimal() + 
  theme(
...
ggplot(month_deaths, aes(x = month, y = total_monthly_deaths, group = 1)) +
  geom_line(size = 1, color = "black") +  # get line graph
  geom_point(size = 3, color = "pink") +  # add points on value in each year
  labs(title = "Monthly Mortality Trends Among Shelter Residents (2007-2023)",
       x = "Month",
...
# Get first 10 lines in the data set as a sample
example_table <- shelter_death_data %>%
  slice(1:10)

example_table %>%
  kbl(caption = "Sample Table of Deaths in Toronto Shelters (2007-2023)",
      col.names = c("ID", "Year", "Month", "Total decedents", "male", "female", "Others")) %>%
...
summary_table <- shelter_death_data %>%
  ...
  summarise(
    ...
    other_ratio = ifelse(any(is.na(shelter_death_data$transgender_non_binary_two_spirit)), NA,round(sum(transgender_non_binary_two_spirit, na.rm = TRUE) / total_deaths * 100, 2))  
